...
def collate_fn_dynamic_padding(batch):
    ...
    for b in batch:
        encoder_inputs.append(b["encoder_input"][:encoder_input_max]) # Truncating to the max str lenggh for the batch
        decoder_inputs.append(b["decoder_input"][:decoder_input_max])
        encoder_mask.append(b["encoder_mask"][0, 0, :encoder_input_max].unsqueeze(0).unsqueeze(0).unsqueeze(0)) # Truncate and then put it inside 4 levels (dimensions) of matrix
        decoder_mask.append(b["decoder_mask"][0, :decoder_input_max, :decoder_input_max].unsqueeze(0).unsqueeze(0))
        label.append(b["label"][:decoder_input_max]) # Label is the tgt so decoder one
...
def get_ds(config):
    ...
    # Filtered and removed longer sentences which are only a few
    # Reduced to 31535 from 32332 i.e. Approx 2.5% of the overall data with seq len <= 100
    filtered_ds = ds_raw.filter(lambda e: ((len(tokenizer_src.encode(e['translation'][src_lang]).ids)<(seq_len-2)) and (len(tokenizer_src.encode(e['translation'][tgt_lang]).ids)<(seq_len-2))))
    ...
    train_dataloader = DataLoader(train_ds, batch_size = config["batch_size"], shuffle = True, collate_fn=collate_fn_dynamic_padding)
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            ...
            batch_iterator.set_postfix({"loss": f"{loss.item():6.3f}", " lr": f"{lrVals[-1][0]:4.8f}", " min loss": f"{min(losses):2.4f}", " average loss": f"{sum(losses)/float(len(losses)):2.4f}"})
            ...
            if not skip_lr_sched:
                scheduler.step()
            lrVals.append(scheduler.get_last_lr())
            
            optimizer.zero_grad(set_to_none=True)
            
            #Update weights
            # Commented for use of scheduler
            # optimizer.step()
            # optimizer.zero_grad(set_to_none=True)
            global_step+=1
